    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                ...
                script {
                    dockerImage = docker.build("your-docker-image:${env.BRANCH_NAME}")
                }
            }
        }
        ...
        stage('Test Docker Image for Vulnerabilities') {
            steps {
                ...
                script {
                    def prismaCloudScanResult = docker.image("your-docker-image:${env.BRANCH_NAME}").inside {
                        sh 'prisma-cloud-scan-command' // Replace with the actual Prisma Cloud scan command
                    }

                    // Fail the build if vulnerabilities are found
                    if (prismaCloudScanResult.contains('high') || prismaCloudScanResult.contains('medium')) {
        ...
        stage('Deploy to Staging') {
            ...
            steps {
                ...
                // Example: Deploy to a testing server or push the Docker image to a staging registry
            }
        }
        ...
        stage('Tag Release for Production') {
            ...
            steps {
                ...
                // Example: Deploy to a production server or update a Kubernetes cluster
            }
        }
         ...
         stage('Push to ECR') {
            ...
            steps {
                script {
                    ...
                    withCredentials([ecr(usernamePassword(credentialsId: 'ECR_CREDENTIALS', passwordVariable: 'ECR_PASSWORD', usernameVariable: 'AWS_ACCESS_KEY_ID'))]) {
